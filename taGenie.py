...
class TAGenie:
    ...
    def generate_task_analysis(self,user_info, org_info):
        user_info=self.provision_user(user_info,org_info)
        self.fetch_schema(user_info['user_org_uid'])
    ...
    def fetch_schema(self, org_uid): return SchemeAnnotator().fetch_schema(org_uid)
